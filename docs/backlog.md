...
| --- | --- | --- | --- | --- |
| Primitive data model cutover | Active trunk | `PDM-GAP-001`, `PDM-GAP-002`, `PDM-GAP-003`, `PDM-GAP-004`, `PDM-GAP-005`, `PDM-GAP-006`, `PDM-GAP-007`, `EXEC-GAP-008` | Replaces the current timing-mode shape with composable primitives that can express the workout types Setmark needs. | Implement the primitive semantics foundation cluster with proof: CoreSession app-runtime semantics, `PDM-GAP-006`, `bug-090`, `bug-089`, and narrow app/server legality parity. History correction is out of scope and remains residual `PDM-GAP-007`; cross-runtime proof is required only if implementation crosses the real app/server sync seam. |
| Today and workout preview | Parallel app lane | `TODAY-GAP-002`, `PREVIEW-GAP-001`, `PREVIEW-GAP-002` | Makes the workout entry path clear, editable where intended, and backed by shared CoreSession execution semantics. | Next move is simulator proof for preview open/dismiss/reopen/Start and boundary rendering. Keep editability proof and edit persistence separate from the read-only preview seam. |
