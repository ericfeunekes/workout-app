...
5. **Relative-load resolution is deterministic at pull time.** When a workout carries a relative-load slot (e.g., "85% of 1RM") and the user parameters carry a timestamped latest value for the referenced parameter, pulling the workout resolves that relative load to its absolute equivalent deterministically from the latest-by-timestamp parameter row, and pins the source parameter's identity alongside the resolved value. Two pulls against different parameter states produce different resolved values.
...
- The relative-load resolution that happens at pull time (server-side resolution).
...
**Out of scope**:
- **Execution of any workout.** The app can now pull, decode, and persist primitive workouts, but it cannot run them. `SessionState`, the session seeder, cursor advancement, rest timing, log-row production, push-queue encoding of log rows, and the execution view-model all remain on the legacy shape in this phase and are Phase 2's scope (straight-sets end-to-end) and Phase 3's scope (all twelve timing modes).
...
- Legacy data migration or preservation. Existing local set_logs and existing server-side prescriptions are dropped when this phase lands (the SwiftData V→V+1 migration is destructive per the dev-mode cutover posture in `docs/specs/primitives-data-model/cutover.md`).
...
3. Relative-load resolution lands after the vocabulary and fixtures, because it consumes the stored workout shape.
