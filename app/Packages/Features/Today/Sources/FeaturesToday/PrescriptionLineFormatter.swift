...
/// match the hi-fi prototype; load is suffixed with " @ <kg>".
public func formatPrescriptionLine(_ prescription: Prescription) -> String {
    ...
    case .straightSets(let sets, let reps, let loadKg, _, _, _, _):
        return straightSetsLine(sets: sets, reps: reps, loadKg: loadKg)
    ...
    case .repRange(let sets, let repsMin, let repsMax, let loadKg, _, _):
        // e.g. "3 × 8–12 @ 60 kg" or "3 × 8–12" when bodyweight-ish
        let repsText = "\(repsMin)\u{2013}\(repsMax)"
        if let loadKg {
            return "\(sets) × \(repsText) @ \(formatKilograms(loadKg)) kg"
        }
    ...
    case .cluster(let sets, let reps, let loadKg, let subSets, _, _):
        // "4 × (3 × 5) @ 100 kg" — condensed cluster summary
        return "\(sets) × (\(subSets) × \(reps)) @ \(formatKilograms(loadKg)) kg"
    ...
    case .amrapToken(let loadKg, _):
        if let loadKg {
            return "AMRAP @ \(formatKilograms(loadKg)) kg"
        }
        return "AMRAP"

    case .warmup(let sets, let reps, let loadKg):
        if let loadKg {
            return "warmup · \(sets) × \(reps) @ \(formatKilograms(loadKg)) kg"
        }
...
private func straightSetsLine(sets: Int?, reps: RepCount?, loadKg: Double?) -> String {
    ...
    let loadText: String? = loadKg.map { "\(formatKilograms($0)) kg" }
