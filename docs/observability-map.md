...
| Log a strength set (load + reps + RIR) | optional pending SetPlan load correction, then SetPlan[idx].done=true, rir populated, `startedAt = workStartedAt anchor`, `completedAt = clock.now`; primitive `.slot` row appended from `ExecutionPlan` | — | `.primitiveSetLogs(PrimitiveSetLog)` enqueued with deterministic primitive coordinate UUID | on flush: upsert `primitive_set_log` by id | `execution.session_mutation (editPendingSet?)`, `execution.session_mutation (logSet)`, `execution.primitive_result_recorded` ✓ |
| Log a primitive slot result | `primitiveSetLogs` appends/upserts one row-shaped `.slot` result with deterministic primitive UUID, primitive IDs, index coordinates, reps/load/unit/duration/distance/RIR as applicable | — | `.primitiveSetLogs(PrimitiveSetLog)` enqueued for live slot logging or included in grouped completion | on flush: upsert `primitive_set_log` by id | `execution.primitive_result_recorded` with role, ids, indices, metric fields, unit, and warmup flag ✓ |
| Log a cardio set (duration / distance / HR / cadence) | SetPlan[idx].done=true, cardio fields populated via `.logCardioSet`; elapsed wins over authored target; primitive `.slot` row appended from `ExecutionPlan` | — | `.primitiveSetLogs(PrimitiveSetLog)` enqueued with deterministic primitive coordinate UUID | on flush: upsert `primitive_set_log` with cardio fields | `execution.session_mutation (logSet, kind=cardio)`, `execution.primitive_result_recorded` ✓ |
...
| AMRAP cap result | view opens the partial-result sheet; save records the current station's partial metric if >0, appends an AMRAP partial note, then routes to next block / complete | — | `.primitiveSetLogs(PrimitiveSetLog)` enqueued only when partial metric >0; prior `next` taps already enqueued completed primitive slot rows | on flush: upsert station-level `primitive_set_log` rows | `execution.session_mutation (logSet)` when partial metric >0, `execution.primitive_result_recorded` ✓ |
| Record primitive aggregate result (AMRAP / for-time / capstone set or block score) | `primitiveSetLogs` appends `.set_result` or `.block_result` with deterministic primitive UUID, aggregate coordinates, and result metrics such as rounds, reps, duration, distance, load/unit | — | included in grouped completion payload | on flush: upsert `primitive_set_log` by id | `execution.primitive_result_recorded` with the same row-shaped payload ✓ |
...
| Tap past-set pill in Rest, edit reps/rir | SetPlan[idx] mutated; original `completedAt` preserved; primitive `.slot` row rebuilt from the active `ExecutionPlan` | — | primitive row re-enqueued with same deterministic primitive coordinate UUID | server updates same `primitive_set_log` row | `execution.past_set_edited`, `execution.primitive_result_recorded` ✓ |
...
| Tap "skip" on RIR sheet | SetPlan[idx].rir = nil, done=true, `skipped=true`; performed metric fields are nil | — | `.primitiveSetLogs(PrimitiveSetLog)` with `skipped=true`, side, unit, and notes overlay preserved | on flush: upsert skipped primitive row | `execution.session_mutation (logSet)`, `execution.primitive_result_recorded` ✓ |
