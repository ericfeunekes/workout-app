...
| FG-005 | `docs/features/execute-loop.md` | Skip action and skip persistence | #4 | partially_built | simulator_observation | Schema cutover | `set_log.skipped` schema/payload foundation exists in Phase 2 with server/contract/package proof and simulator launch proof; active/rest skip affordances remain later. |
...
| FG-014 | `docs/features/execute-loop.md`, `docs/features/history.md` | Per-side logging | #19 | partially_built | simulator_observation | Schema cutover | `set_log.side` schema/payload foundation exists in Phase 2 with server/contract/package proof and simulator launch proof; app two-row semantics and server aggregate semantics remain later. |
