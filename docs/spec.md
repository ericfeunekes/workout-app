...
- **Save & done** → one local completion record; local cache and REST completion publisher consume that same record.
...
| save-and-done | [save-and-done](features/save-and-done.md) | `tested` | `testSaveAndDoneInvokesLocalCompletionWriterOnce`, `testSaveAndDoneEnqueuesCompletionExactlyOnce`, `testCompleteAloneDoesNotEnqueueStatus`, `testSaveAndDoneWritesNoteToCompletedWorkout`, `testSaveAndDoneEmptyNoteCollapsesToNil`, `testSaveAndDoneEnqueuesBodyweightUserParameter`, `testSaveAndDoneNilBodyweightDoesNotFire`, `testSaveAndDoneReEntrancyGuardDropsDoubleTap` (bug-044). Sync tests cover grouped completion encoding, retry retention, 2xx removal, and dedup. Server `test_sync_results_mixed_ownership_rejects_foreign_items` proves combined set-log/status rollback. Dictation-mic on the note deferred. |
