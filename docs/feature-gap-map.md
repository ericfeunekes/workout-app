...
| `WATCHKIT-GAP-002` | `docs/features/watch-workoutkit-handoff.md` | `paceTargetRun` now maps a single scheduled running distance+duration slot to WorkoutKit `PacerWorkout`; `WorkoutKitHandoff` owns the proof-gated Today preview action, latest-attempt/receipt persistence, and telemetry. Remaining work is release-candidate real-device proof before normal TestFlight exposure. | `proof` |
