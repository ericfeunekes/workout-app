...
**Relative-load resolution (Q-F / Q-J) still uses `current-local-value-at-seed-time`.** When the app seeds a pulled workout into an `ExecutionPlan`, `(0.85, "1rm", relative)` looks up `one_rep_max_<exercise_id>_kg` from the latest locally mirrored `user_parameters` row and caches the absolute kg on `ExecutionSlot.load_kg`. The current app runtime does not pin the source `user_parameters` row id on `ExecutionSlot`; that provenance field is deferred until a coordinated execution-plan/schema cutover needs it. Audit of "what load did this workout use?" still works via the cached absolute load and authored relative-load reference.
