...
| bug-101 | Health Archive Settings controls render but activation lacks end-to-end proof | Settings / HealthKit archive export | open | HealthKit export QA on 2026-05-19 proved the signed HealthKit archive projection and server ingestion paths, and video/screenshots proved the Settings Health Archive section renders scope, descriptor subset, automatic, next-attempt, status, and Export Now rows. Follow-up XCUITest/debugging found the Settings rows are addressable only after adding accessibility IDs, but descriptor/automatic activation and Export Now status change still do not produce a deterministic UI-level state change in simulator QA. Treat as a P2 Settings interaction/proof blocker before claiming the Settings export surface fully verified. |

**ID allocation:** bug-001–060 were assigned during the initial QA campaign; `bug-061+` are post-campaign findings. `qa-001`–`qa-047` were campaign-internal ids; any finding that still matters as an active defect must have a `bug-NNN` row here. Next ID: `bug-102`.
