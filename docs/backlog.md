...
| --- | --- | --- | --- | --- |
| Primitive data model cutover | Active trunk | `PDM-GAP-002`, `PDM-GAP-003`, `PDM-GAP-004`, `PDM-GAP-006`, `PDM-GAP-007`, `PDM-GAP-008`, `EXEC-GAP-008` | Replaces the current timing-mode shape with composable primitives that can express the workout types Setmark needs. | Primitive wire/schema surfaces, QA-data reset posture, sync/readback proof, result role query-safety, no-work rejection, and primitive-only grouped completion pushes are implemented. No production migration or data-preservation lane remains: the next deployment may delete and recreate the server database, and local QA cache data may be reset. Remaining scope is residual runtime semantics where the app still bridges into legacy execution, especially History correction, completion/correction summaries for non-rep metrics, and unsupported primitive repeat/cell shapes. |
...
| App sync ownership and foreground lifecycle | Parallel sync proof lane | `TEST-GAP-004` | Foreground pull, cache writeback, push flusher lifecycle, token recovery, and stage telemetry now have one Shell-owned coordinator; remaining work is proving the running app's lifecycle hook. | Use simulator/app-root evidence to prove `scenePhase` active/background transitions invoke `AppSyncCoordinator` correctly. |
...
| Settings and recovery surfaces | Parallel app lane | `SETTINGS-GAP-002`, `SETTINGS-GAP-003`, `SETTINGS-GAP-004` | Gives sync recovery, server changes, reset, units, telemetry diagnostics, and autoreg defaults one coherent user-facing surface. | Keep section-as-type; prove destructive confirmations, token recovery, and accessibility before adding new rows. Related telemetry, bootstrap, and autoreg gaps stay in their owning lanes. |
...
| Release hardening | Parallel release lane | `RELEASE-GAP-001`, `RELEASE-GAP-002`, `RELEASE-GAP-003` | Turns the bootstrap TestFlight path into a repeatable, auditable personal release stage instead of command memory and broad temporary credentials. | Implement a narrow local release target/runbook with preflight, archive/export/upload, App Store Connect readback, version bump guardrails, dedicated signing-keychain posture, and least-privilege upload-key rotation. |
...
| HealthKit data access | Proof/follow-on lane | `HKDATA-GAP-003`, `HKDATA-GAP-005`, `TEST-GAP-003` | HealthKit batch/archive core is implemented for the first supported registry through `HealthKitBridge` and the local SwiftData archive projection. Package and Persistence tests cover typed request/query behavior, SwiftData projection, tombstone-applied current records, migration behavior, deterministic fake live streams, readable metric replay traces, watch HR rendering, and outbound HR payloads. The signed iOS simulator archive proof now passes through `make test-healthkit-ui`, and XcodeBuildMCP watch simulator proof captures HealthKit authorization, live metric ticks, and builder save. | Next missing product seam is phone-side consumption/persistence of watch metric messages. Real sensor/sync claims still require physical iPhone + Apple Watch diagnostics. |
| Early WorkoutKit handoff | Evidence lane | `WATCHKIT-GAP-002`, `WATCHKIT-GAP-004` | Shorter Apple Watch path: push eligible workouts to Apple's Workout app. | The pure export profile/classifier and `WorkoutKitAdapter` backend now exist. The adapter has proof-gated production APIs and DEBUG diagnostics, and production push blocks value-backed payloads until exact target mapping exists. Product export still waits on export tracking persistence and real-device schedule/open proof from a trusted iPhone paired to a real Apple Watch. |
