...
| --- | --- | --- | --- | --- |
| Primitive data model cutover | Active trunk | `PDM-GAP-001`, `PDM-GAP-002`, `PDM-GAP-003`, `PDM-GAP-004`, `PDM-GAP-005`, `PDM-GAP-006`, `PDM-GAP-007`, `EXEC-GAP-008` | Replaces the current timing-mode shape with composable primitives that can express the workout types Setmark needs. | Next phase is the primitive semantics foundation: centralize composition/result rules in CoreSession, then route `PDM-GAP-006`, `bug-089`, and `bug-090` fixes through that foundation before expanding the full matrix. |
| Today and workout preview | Parallel app lane | `TODAY-GAP-002`, `TODAY-GAP-003`, `PREVIEW-GAP-001`, `PREVIEW-GAP-002`, `PREVIEW-GAP-003` | Makes the workout entry path clear, editable where intended, and backed by the same execution projection used by Active. | Route projection-backed current/remaining/upcoming preview content before expanding edit/persistence behavior; keep simulator proof on the preview flow before marking it verified. |
