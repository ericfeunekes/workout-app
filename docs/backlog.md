...
| --- | --- | --- | --- | --- |
| Primitive data model cutover | Active trunk | `PDM-GAP-002`, `PDM-GAP-003`, `PDM-GAP-004`, `PDM-GAP-005`, `PDM-GAP-006`, `PDM-GAP-007`, `PDM-GAP-008`, `EXEC-GAP-008` | Replaces the current timing-mode shape with composable primitives that can express the workout types Setmark needs. | Current cluster is in closeout/review: primitive wire/schema surfaces, explicit QA-data reset, sync/readback proof, result role query-safety, and no-work rejection are implemented. Remaining scope is to close or narrow residual composition semantics where the app still bridges into legacy execution, especially History correction and unsupported primitive repeat/cell shapes. |
...
| HealthKit data access | Requirements-to-implementation lane | `HKDATA-GAP-001`, `HKDATA-GAP-002`, `HKDATA-GAP-003`, `TEST-GAP-003` | Makes HealthKit data available through one typed module for full archive export, post-workout readback, and live workout consumers. Simulator proof covers batch/query mechanics; live Watch claims stop at the real-device diagnostic boundary. | Implement descriptor-to-query mapping and local archive projection next. Stop before user-facing live Watch behavior until the physical iPhone + Apple Watch diagnostic proves live sensor delivery. |
| Early WorkoutKit handoff | Evidence lane | `WATCHKIT-GAP-002`, `WATCHKIT-GAP-004` | Shorter Apple Watch path: push eligible workouts to Apple's Workout app. | The vendor-neutral export profile and fake-backed WorkoutKit classifier now exist as a pure package. Scratch real-device schedule/open probes now typecheck, but the real-device run is blocked until a trusted iPhone paired to a real Apple Watch is visible to Xcode; only then can we settle visibility/startability and duplicate/update behavior before any user-facing export. |
